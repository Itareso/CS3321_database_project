...
from init_recom import VectorDBBookRecommender
...
async def setup_recommender(app, loop):
    # 在服务器启动前初始化推荐器
    app.ctx.recommender = VectorDBBookRecommender(initialize=False)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Sanic server port")
    parser.add_argument(
        "--port", type=int, default=8088, help="Port to run the server on"
    )
    args = parser.parse_args()

    app.run(host="0.0.0.0", port=args.port, dev=True)
